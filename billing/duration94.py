            ...
            try:
                with open(lookup_file, 'r') as f:
                    ...
                    for row in reader:
                        ...
                        pi_name = row['PIName']
                        lookup_dict[study_id] = {'FundCode': fund_code, 'PIName': pi_name}
                logger.info(f"Loaded {len(lookup_dict)} entries from billing lookup table at {lookup_file}")
                return lookup_dict
            except Exception as e:
                logger.warning(f"Failed to load billing lookup table from {lookup_file}: {e}")
                continue
    
    # If no file found, log warning and return empty dict
    logger.warning("No billing lookup table found in /94tresearch/billing or current directory")
    return lookup_dict


def studies_for_date(study_date):
    query = {"StudyDate": study_date, "AccessionNumber": "9T*"}
...
def duration(study):
    ...
    try:
        return max(instance_creation_datetimes) - min(instance_creation_datetimes), min(instance_creation_datetimes)
    except ValueError:
        return 0, None
...
def calculate_invoice_number(scan_date):
    ...
    months_diff = (scan_date_obj.year - may_2025.year) * 12 + (scan_date_obj.month - may_2025.month)
...
def get_study(study_id):
    ...
    study_id_part = study_id_tag.split('_')[-1][:6] if '_' in study_id_tag else study_id_tag[:6]
