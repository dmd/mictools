...
if __name__ == "__main__":
    ...
    if SYSTYPE == "sge":
        ...
        sub_cmd = f"{SBATCH} --job-name {args.jobname} --output={logsdir}/%x-%j.out --error={logsdir}/%x-%j.err --time 1-4 --cpus-per-task={args.ncpus} --mem={args.ramsize*1.25}G {filename}".split()
    print(" ".join(sub_cmd))
    if args.dry_run:
